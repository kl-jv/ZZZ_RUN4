...
/****** Object:  View [dbo].[vw_PriceBooks_changed]    Script Date: 20/12/2023 12:25:50 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[vw_PriceBooks_changed] AS
...
--  One Pricebook : 400000004
	 CAST ('400000004' AS VARCHAR(9)) AS PriceBook
	...
	,CASE WHEN pri.LF_CODICE_FASE  = 0 THEN 10 ELSE 20 END AS PriceType						/*Price Type (prty) ** Only applicable when Module not Sales | TRUE | 5 |  Byte | | 5;"Not Applicable (Default)";10;"Buying";15;"Item Subcontracting";20;"Operation Subcontracting";25;"Service Subcontracting"|*/
	,CAST(CONVERT(DATETIME,CONVERT(INT,pri.LF_DATA_DECORR-1)) AS DATETIME) AS EffectiveDate		/*Effective Date (efdt) | FALSE | null |  Date/Time | | |*/
/* Missing IN Template 4 fileds  */ 
--	, 0 AS PriceBookMPNLine																		-- ( prbl, long )
--	, 0 AS PriceBookLine																		-- ( pbln, long ) 
--	, CAST (NULL AS VARCHAR(35)) AS ManufacturerPartNumber										--- (  mpnr, string , 35  )
--	, CAST (NULL AS VARCHAR(6)) AS Manufacturer													-- (cmnf , string , 6 ) 

	,1 AS BreakType																				/*Break Type (brty) | FALSE | 1 |  Text |10 | 1;"Minimum (Default)";2;"Up To"|*/
	,CAST(CONVERT(DATETIME,CONVERT(INT,pri.LF_DATA_TERMINE-1)) AS DATETIME) AS ExpiryDate		/*Expiry Date (exdt) - If no time is specified, time will be 00:00. You can specify the time after the date e.g. 23:59:59 | FALSE | null |  Date/Time | | |*/
	,CAST(GETDATE() AS DATETIME) AS EntryDate													/*Entry Date (endt) | TRUE |  |  Date/Time | | |*/
	...
	,CAST(NULL AS VARCHAR(30)) AS [Source]														/*Source (sour) | FALSE | cast(null as varchar(30)) |  Text |30 | |*/
...
AND bp.[Business Partner] IS NOT NULL
