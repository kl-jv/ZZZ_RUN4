...
/* IT0110 and IT0210 for  POV1  from SCModelMap*/ 
...
--		kps.POV1 = 1															
/* 07-03-2024 KL:  SITE IT0100 was also selected with POV1 <> 0  */ 
		pha.phantom = 2
		and kps.POV1 <> 1 
...
/* 29-02-2024 KL:  Map warehouse to warehouse from ZZZ_Italy.dbo.SCModelMap (scm) */ 
...
/* 04-03-2024 KL:  POV2 and RD not required anymore */ 	
							-- 	JOIN ZZZ_Italy.dbo.KPSource kps ON (bom.DB_CODICE_PADRE = kps.Item AND bom.DB_DITTA = 1 AND kps.Migrate = 1 AND (kps.POV1 = 1 OR kps.POV2 = 1 OR kps.RD = 1))
								JOIN ZZZ_Italy.dbo.KPSource kps ON (bom.DB_CODICE_PADRE = kps.Item AND bom.DB_DITTA = 1 AND kps.Migrate = 1 AND (kps.POV1 <> 0 ))
...
/* 04-03-2024 KL:  Join added to kps to get phantom value , based on ItemGroup,Supply Source */ 
				JOIN ZZZ_Italy.dbo.KPsource kps ON ( kps.Item = itm.MG_CODICE )
...
/*  IT.EXT.05       (IT.EXT.04 Country ROU not valid anymore ) */ 
