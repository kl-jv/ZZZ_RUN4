...
/****** Object:  View [dbo].[vw_JobShopRouting]    Script Date: 3/8/2024 10:50:46 AM ******/
SET ANSI_NULLS ON
GO
...
/* For Spare Parts (POV3), it is depending on the fact if an item is used as phantom in the bill of material, it should not have routing */
...
FROM
	(
		...
		/* 30-01-2023 KL : Not New field Revision in template 202310 was already in april template */
			,CAST('000001' AS VARCHAR(6)) AS Revision	
			,20 AS Status																/*Status (rost)  When loading with DAL, use status 10 (new) and approve as post conversion action. | FALSE | 20 |  | 10;"New";20;"Approved";30;"Expired"|*/
			,CAST('2023/01/01 00:00:00' AS SMALLDATETIME) AS EffectiveDate				/*Effective Date (efdt) | FALSE | null |  | |*/
			,CAST(NULL AS SMALLDATETIME) AS ExpiryDate									/*Expiry Date (exdt) - When empty, date will be set to MAX | TRUE | null |  | |*/
					...
					/* because of duplicates in work center, duplicates removed */
						LEFT JOIN
							(
								...
								FROM
									KPRAKTOR.SIAPR.CICLIL rou
									JOIN ZZZ_Italy.dbo.KPSource kps ON (rou.CL_CODICE_CICLO = kps.Item AND rou.CL_DITTA =  1 AND kps.Migrate = 1 AND POV3 = 1)
								WHERE
									rou.CL_DATA_FIN_VAL >= CONVERT(INT,GETDATE()) AND CL_CODICE_CENTRO <> 'E'
								GROUP BY 
									CL_CODICE_CICLO
