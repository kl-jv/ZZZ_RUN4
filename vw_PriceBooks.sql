...
/****** Object:  View [dbo].[vw_PriceBooks]    Script Date: 3/8/2024 11:09:28 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[vw_PriceBooks] AS


SELECT
	 CAST ('400000004' AS VARCHAR(9)) AS PriceBook												/*Price Book (prbk)   - Reference to  tdpcg011                                                                                                   ERPLN table: tdpcg031 | TRUE | cast(null as varchar(9)) |  Text |9 | |*/
	...
	,CAST(CASE WHEN pri.LF_CODICE_FASE  = 0 THEN 10 ELSE 20 END AS FLOAT) AS PriceType				/*Price Type (prty) ** Only applicable when Module not Sales | TRUE | 5 |  Byte | | 5;"Not Applicable (Default)";10;"Buying";15;"Item Subcontracting";20;"Operation Subcontracting";25;"Service Subcontracting"|*/
	...
	,CAST(NULL AS SMALLDATETIME) AS ExpiryDate													/*Expiry Date (exdt) - If no time is specified, time will be 00:00. You can specify the time after the date e.g. 23:59:59 | FALSE | null |  Date/Time | | |*/
	,CAST(GETDATE() AS SMALLDATETIME) AS EntryDate													/*Entry Date (endt) | TRUE |  |  Date/Time | | |*/
...
FROM
	...
	JOIN ZZZ_Italy.dbo.BP_Conv bp ON (pri.LF_CODICE_CLIFOR = bp.KPraktor AND pri.LF_DITTA = 1)
